      ...
      <Card.Root>
        ...
        <Card.Content>
          <div class="mt-2 space-y-3">
            ...
            <Progress value={calculateProgress(goal.currentAmount, goal.targetAmount)} class="h-2" />
        ...
        <Card.Footer class="flex justify-between border-t bg-muted/50 p-3">
          ...
          <Button variant="ghost" size="icon" class="text-destructive" onclick={() => deleteGoal.mutate(goal.id)}>
...
{#if goals.data?.length === 0}
  <div class="flex h-50 flex-col items-center justify-center rounded-lg border border-dashed text-center">
