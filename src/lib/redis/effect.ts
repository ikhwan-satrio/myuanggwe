...
export const RedisLive = Layer.sync(RedisService, () => new Redis({
...
const cacheGet = <T>(key: string) => Effect.gen(function* () {
...
const cacheSet = (key: string, value: unknown, ttl: DurationInput = Duration.minutes(10)) => Effect.gen(function* () {
  const redis = yield* RedisService
  ...
  Effect.catchAll((e) => log.error(e))
)

const cacheDel = (key: string) => Effect.gen(function* () {
  const redis = yield* RedisService
  yield* Effect.tryPromise({
    try: () => redis.del(key),
...
const cacheInvalidate = (pattern: string) => Effect.gen(function* () {
  const redis = yield* RedisService
...
const cacheExists = (key: string) => Effect.gen(function* () {
  const redis = yield* RedisService
...
const cacheTtl = (key: string) => Effect.gen(function* () {
  const redis = yield* RedisService
  return yield* Effect.tryPromise({
    try: () => redis.ttl(key),
...
export const withBackendCache = <T>(key: string, fetcher: () => Promise<T>, ttl: DurationInput = Duration.minutes(10)): Effect.Effect<T, never, RedisService> => Effect.gen(function* () {
  ...
  .pipe(
    Effect.catchTag("CacheError", (e) =>
      Effect.zipRight(
        log.error(e),
        Effect.tryPromise({ try: fetcher, catch: (cause) => new CacheError({ operation: 'fallback', key, cause }) })
      )
    ),
    Effect.orDie
  )
