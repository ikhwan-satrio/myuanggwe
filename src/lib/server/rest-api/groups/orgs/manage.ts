...
export const manageOrgsGroup = createServer({ name: 'manage-orgs', prefix: '/manage-orgs' })
	...
	.delete(
		'/members/:id',
		async ({ params, activeOrg, user, status }) => {
	...
	.put(
		'/members/:id/role',
		async ({ params, body, activeOrg, user, status }) => {
	...
	.delete(
		'/',
		async ({ activeOrg, user, status, request }) => {
			if (!activeOrg) return status(400);
			...
			try {
				await db.transaction(async (tx) => {
					...
					await tx
						...
						.where(eq(schema.transactions.organizationId, activeOrg.id));
					await tx.delete(schema.wallets).where(eq(schema.wallets.organizationId, activeOrg.id));
					await tx
						.delete(schema.categories)
						.where(eq(schema.categories.organizationId, activeOrg.id));
